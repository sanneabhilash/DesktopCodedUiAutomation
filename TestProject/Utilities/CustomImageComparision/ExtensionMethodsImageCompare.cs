...
{
    public static class ExtensionMethodsImageCompare
    {
        ...
        static ExtensionMethodsImageCompare()
        {
            for (int i = 0; i < 256; i++)
            {
        ...
        static readonly ColorMatrix ColorMatrix = new ColorMatrix(new[]
        {
            new[] {.3f, .3f, .3f, 0, 0},
            new[] {.59f, .59f, .59f, 0, 0},
            new[] {.11f, .11f, .11f, 0, 0},
        ...
        /// <param name="threshold">How big a difference (out of 255) will be ignored - the default is 3.
        /// Threshold = 0 => 52%, 1=> 7%, 2 => 1% and 3=> 0%</param>
